...
ANCHORS = {
    ...
    # ILM 从 7 脚水平向右离开，避免旧位置的回路线包围下方 VBUS 输出。
    "R_EFUSE_ILM": (37.5, 18.75, 0),
    # 固定前馈电容；否则 eFuse 区调整后自动排布可能把它放到预布的 VBUS 回路上。
    "C_FF": (41.75, 18.75, 0),
    "J_LCD_L": (47.0, 2.5, 90),
    "J_LCD_R": (47.0, 7.5, 90),
    # 背光高边开关放在双屏排针正下方的空区：LCD_BL 只需 4~11mm 就能到两排 8 脚，
    # 而 GPIO48 的 25kHz 栅极信号可以跟着已有的 LCD_DC/LCD_CS 走廊横穿板子。
    # 此处 x<62，In2.Cu 的 +3V3 覆铜仍在，源极和上拉都能就近打孔取电。
    "Q_LCD_BL": (60.0, 13.0, 0),
    "R_LCD_BL_GATE": (56.8, 12.05, 0),
    "R_LCD_BL_OFF": (57.0, 10.3, 0),
    # 中部：传感器
    "U_IMU": (46.0, 30.0, 0),
    "U_TOUCH": (30.0, 46.0, 0),
    "J_TOUCH": (19.0, 56.0, 0),
...
def safety_silk(fps) -> tuple[tuple[str, float, float, str], ...]:
    ...
    return (
        ...
        # 两行 5V 标签左对齐排在 J_VMOT 上方、H3 安装孔左侧。2026-09-14 评审：原位置
        # 排在两排圆屏排针末端，容易被读成屏幕座是 5V 电机电源，因此要求离屏排针焊盘
        # 至少 7mm。屏排针从 7 针加到 8 针后 8 脚右移 2.54mm，两行必须整体右挪；右边
        # 又顶着 H3 安装孔，所以第二行去掉与第一行重复的「5V」，只留设计方案要求的
        # 「电机/加热专用」。
        ("VMOT 仅限 5V", *at_ref("J_VMOT", -10.3, -13.4), "left"),
        ("电机/加热专用", *at_ref("J_VMOT", -10.3, -11.5), "left"),
