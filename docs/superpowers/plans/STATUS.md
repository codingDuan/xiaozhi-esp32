...
1. **验证双眼眨眼同步**
   用户已确认眼睛颜色和造型正常；静态照片不能证明双眼眨眼时序，仍需动态观察。
...
4. **配网二维码实机扫码** —— 软件已实现并在实机触发，左眼显示二维码、右眼显示等待环；需手机确认真屏可扫。
5. **OTA/Assets 进度环实机验证** —— 需一次真实下载确认进度推进、完成/失败后恢复眼睛。
6. **舵机 trim 校准** —— 装进玩偶后「手臂自然下垂」未必对应 0°，
   建议存 NVS `Settings` 以便在线调整。
7. **独立 5V 电源** —— 实测双路无间歇连续动作会 BROWNOUT。
