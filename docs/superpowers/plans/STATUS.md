...
| MCP 手势工具 ×3 | ⚠️ | hug 已实机通过；cheer 的 LLM→设备 RPC 成功、待观察动作；wave 待验 |
| 状态反射 `PlushBehavior` | ✅ | 状态轮询生效 |
| emotion → 表情+手势 | ⚠️ | 白名单运行样本已通过；眼型与物理手势同时呈现仍待现场观察 |
...
| 服务端 prompt | ✅ | 21 项映射一致；重启后模型连续产出白名单内 `😂`、`😌`，不再恒为 happy |

## 二、待办

### 已收口决定

emoji 白名单已从既有运行日志得到样本验证：prompt 重启后的同一会话连续产出 `😂` 与 `😌`，二者都在白名单内。prompt 属于概率约束，今后若再发现越界 emoji，再升级为服务端确定性校验；当前不把这个假设性加固扩入固件交付。

双眼眨眼不再列为待办：用户已接受当前眼睛表现并要求停止排查。SPI 保持已实机稳定的 10MHz；只有未来重新观察到可见不同步或流畅度不足时，才考虑上探 20MHz。

### 仍需完成

1. **配网二维码实机扫码** —— 软件已实现并在实机触发，左眼显示二维码、右眼显示等待环；需手机确认真屏可扫。
2. **OTA/Assets 进度环实机验证** —— 需一次真实下载确认进度推进、完成/失败后恢复眼睛。
3. **舵机 trim 校准** —— 装进玩偶后「手臂自然下垂」未必对应 0°，
   建议存 NVS `Settings` 以便在线调整。
4. **独立 5V 电源** —— 实测双路无间歇连续动作会 BROWNOUT。
   装进玩偶带载后必需，需 1000µF 电容。
5. **稳定服务端地址** —— 已确认固件支持 mDNS，并选定 `MacBook-Pro-107.local`；
   仍需重新配置设备 OTA 地址并在实际热点上验证 mDNS 转发。
...
| GPIO | 用途 | 备注 |
|---|---|---|
| 0 | BOOT 按键 | 主功能键：短按对话、长按 2s 进出待机 |
| 1 / 2 / 42 | 麦克风 WS / SCK / DIN | 2026-09-15 改外接，走 J_MIC |
...
| 47 | 眼屏 DC | 两屏并联 |
| 48 | 双眼背光 PWM | 驱动高边 P-MOS，低电平点亮；原板载 WS2812 已取消 |
...
- **2026-09-08 现状**：服务端仍监听 `:8003` / `:8010`，但日志最后一次设备连接使用
  `172.20.10.14`；Mac 当前地址已是 `10.35.80.13`，需重新配网或改用稳定地址后才能继续 B2/B3 与真实 OTA 验收。
- **稳定地址候选**：Mac 的 Bonjour 名称为 `MacBook-Pro-107.local`，本机可解析到当前地址；
  plush-toy 构建中的 lwIP 已开启 mDNS 查询。设备 OTA 地址可改为
  `http://MacBook-Pro-107.local:8003/xiaozhi/ota/`，服务端下发的 WebSocket 地址可固定为
  `ws://MacBook-Pro-107.local:8010/xiaozhi/v1/`。这仍需在实际热点上验证，因为热点可能隔离 mDNS。
