...
**执行状态（2026-09-08）：** Tasks 1–5 已实现、提交并经过主机/实机构建验证；用户已接受当前眼睛表现并要求停止专项排查，因此保留实机稳定的 10MHz，不再为未观察到的问题尝试提速。Tasks 6–7 的二维码与 overlay 已实现并提交（`5693891`），主机测试及 ESP-IDF 6.1 构建通过，但最新并发加固镜像尚未重新烧录。二维码像素输出已由 macOS Vision 成功回读，真屏手机扫码仍待验收。下面复选框已按现有证据同步，未勾选项不得视为完成。
