...
### 量总线健康：`tools/plush_toy_i2c_trial.py`

换上拉、改走线、换供电之后想知道"到底有没有变好"，用这个脚本，别手工看 `status`。

```sh
python3 tools/plush_toy_i2c_trial.py --label "面包板原状"     # 跑一档
python3 tools/plush_toy_i2c_trial.py --label "板载4.7k"       # 换硬件后再跑
python3 tools/plush_toy_i2c_trial.py --compare                # 并排比
```
...
| 指标 | 怎么测 | 说明什么 |
|---|---|---|
| 空闲丢帧率 | 首尾各读一次 `status`，中间完全不碰总线 | 总线固有质量，**唯一可用于跨配置比较的指标** |
| 拥塞读失败率 | 连续读 `status`，数 `accel` 字段缺失次数 | 重负载下的读成功率，只能同负载互比 |

**每个结果都带 95% 置信区间，区间重叠时脚本会明确拒绝宣布赢家。** 这不是谨慎，是有过教训：2026-09-11 的排查里，先用"边高频轮询边看丢帧率"得出「降低 I2C 时钟有效」（100kHz→20kHz 读失败率 21.7%→3.3%），后来发现那个指标被轮询自身的总线拥塞污染了；干净重测下 100k/50k/20kHz 三档空闲丢帧率分别是 9.8% / 11.5% / 9.1%，**完全无差别**，结论被推翻。同一天内同一套硬件的空闲丢帧率还测到过 3.9% 与 9.8%，相差一倍。

因此：
- 判断改动有效，看的是**区间不重叠**，不是看哪个数字小
- 区间太宽就加长 `--idle-seconds`，别拿单次读数下结论
- 跨 trial 比较前确认 `touch_modes_during` 一致（脚本默认测量期间置 0 并在结束时还原，就是为了统一总线负载）
