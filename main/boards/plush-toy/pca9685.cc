...
uint8_t Pca9685::ReadPrescale() {
    ...
    uint8_t reg = kRegPrescale, out = 0;
    if (i2c_master_transmit_receive(dev_, &reg, 1, &out, 1, kTimeoutMs) != ESP_OK) {
        return 0;
    }
    return out;
}
...
bool Pca9685::Init(int freq_hz) {
    ...
    if (!WriteReg(kRegMode1, kMode1Sleep)) return false;
    vTaskDelay(pdMS_TO_TICKS(5));

    uint8_t prescale = (uint8_t)((25000000.0 / (4096.0 * freq_hz)) + 0.5) - 1;
    if (!WriteReg(kRegPrescale, prescale)) return false;

    if (!WriteReg(kRegMode1, kMode1AutoInc | kMode1AllCall)) return false;
    vTaskDelay(pdMS_TO_TICKS(5));
    if (!WriteReg(kRegMode1, kMode1Restart | kMode1AutoInc | kMode1AllCall)) return false;
    if (!WriteReg(kRegMode2, kMode2OutDrv)) return false;

    uint8_t back = ReadPrescale();
    if (back != prescale) {
        ESP_LOGE(TAG, "PRE_SCALE 回读 %u，期望 %u —— 芯片未正确响应（时钟过高或接线不良）",
                 back, prescale);
        return false;
    }
...
void Pca9685::SetPulseUs(int ch, int us) {
    ...
    i2c_master_transmit(dev_, buf, sizeof(buf), kTimeoutMs);
}
