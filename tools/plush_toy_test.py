...
# 不设默认 IP。板子的地址随 DHCP 变，写死的默认值只会让人对着一个早就失效的
# 地址反复超时，还分不清是"地址错了"还是"通道没起来"。必须显式给出。
DEVICE_URL_ENV = "PLUSH_TOY_DEVICE_URL"
DEVICE_URL_HELP = (
    "board HTTP test URL, for example http://192.168.1.23:8181; "
    f"or set ${DEVICE_URL_ENV}. 取 IP 的方法见 main/boards/plush-toy/README.md"
)
REGRESSION_CASES = [
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--device-url", default=os.environ.get(DEVICE_URL_ENV),
                        help=DEVICE_URL_HELP)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.device_url:
        parser.error(
            f"--device-url is required (or set ${DEVICE_URL_ENV}). "
            "板子 IP 随 DHCP 变，取法见 main/boards/plush-toy/README.md「怎么拿到板子 IP」"
        )
    try:
